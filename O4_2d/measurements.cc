...
void Measurements::PrintfSpinCorrelation(const O4& O4, MPS& psi)
{
  ...
  auto Nx = O4.Nx;
  auto Ny = O4.Ny;
  auto file = format("data/spin_Nx%dNy%d_a2%.1f_a3%.1f_U%.5f.dat", Nx, Ny, O4.a2, O4.a3, O4.U);
  //println(psi);
...
void Measurements::PrintfSpinxCorrelation(const O4& O4, MPS& psi)
{
  ...
  auto Nx = O4.Nx;
  auto Ny = O4.Ny;
  auto file = format("data/spinx_Nx%dNy%d_a2%.1f_a3%.1f_U%.5f.dat", Nx, Ny, O4.a2, O4.a3, O4.U);
  if (!utils::fileExists(file) || new_measurements)
    {
      auto sites = O4.sites;
      std::ofstream ofile;
...
void Measurements::PrintfDimerCorrelation(const O4& O4, MPS& psi)
{
  ...
  auto Nx = O4.Nx;
  auto Ny = O4.Ny;
  auto file = format("data/dimer_Nx%dNy%d_a2%.1f_a3%.1f_U%.5f.dat", Nx, Ny, O4.a2, O4.a3, O4.U);
  if (!utils::fileExists(file) || new_measurements)
    {
      auto sites = O4.sites;
      std::ofstream ofile;
...
void Measurements::PrintfDimer2Correlation(const O4& O4, MPS& psi)
{
  ...
  auto Nx = O4.Nx;
  auto Ny = O4.Ny;
  auto file = format("data/dimer2_Nx%dNy%d_a2%.1f_a3%.1f_U%.5f.dat", Nx, Ny, O4.a2, O4.a3, O4.U);
  if (!utils::fileExists(file) || new_measurements)
    {
      auto sites = O4.sites;
      std::ofstream ofile;
...
{
  auto file = format("data/entropy_Nx%dNy%d_a2%.1f_a3%.1f_U%.5f.dat", O4.Nx, O4.Ny, O4.a2, O4.a3, O4.U);
  if (!utils::fileExists(file) || new_measurements)
    {
      std::ofstream ofile;
      ofile.open (file);
  ...
  for (int i=1; i<=O4.N; ++i)
    {
      ...
      //auto stop = std::chrono::high_resolution_clock::now();
      //auto duration = std::chrono::duration_cast<std::chrono::seconds>(stop - start);
      printfln("i = %d, spin correlation = %.10f", i, spin_num);
      //std::cout << "Total spin time = " << duration.count() << " seconds\n";
    }
}

void Measurements::PrintxDimer(const O4& O4, MPS& psi)
{
  for (int i=1; i<=O4.N; ++i)
    {
      auto dimer_mpo = Dimer(O4.sites, 1, 3, i, (i+1)%O4.N+1);
...
void Measurements::PrintyDimer(const O4& O4, MPS& psi)
{
  for (int i=1; i<=O4.N; i+=2)
    {
      auto dimer_mpo = Dimer(O4.sites, 1, 2, i, i+1);
      auto dimer_num = inner(psi, dimer_mpo, psi);
